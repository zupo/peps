...
C without any additional dependencies.
...
support for the WAV sound format. The WAV format has been largely superseded
by better codes that support lossless audio compression, e.g. FLAC.

The *wave* module one of the three modules besides `aifc`_ and `sunau`_,
that depend on `audioop`_. With deprecation of the other multimedia formats,
it would be the last multimedia module.
...
documentation may be moved into a new git repository. The idea is to have
a place from which community members can pick up and fork code.
